...
class ArticleController extends Controller
{
    ...
    public function update(Request $request, Article $article)
    {
        ...
        // funcion para quitar los comentarios del archivo .tex
        ...
        // funcion para cambiar el titulo del archivo .tex
        ...
        // borra el \section{FUNDATION} reemplaza directamente por my_tex_content
        function replace_tex_content2($file, $my_tex_content)
        {
            $content = file_get_contents($file);
            $content = str_replace('\section{FUNDATION}', $my_tex_content . "\n\n" . '\bibliographystyle{plain}' . "\n" . '\bibliography{References.bib}' . "\n", $content);
            //$content = str_replace('\section{FUNDATION}', $my_tex_content . "\n\n" . '\bibliography{References.bib}' . "\n", $content);
        ...
        if($request->template == null){
            ...
            $extracted_files = File::files(public_path('articles_public/' . $article->id));

            // Definir las extensiones a eliminar
            $extensions_to_delete = ['aux', 'bbl', 'blg', 'log', 'out'];

            // Recorrer cada archivo extraído
            foreach ($extracted_files as $file) {
                // Obtener la extensión del archivo
                $extension = pathinfo($file, PATHINFO_EXTENSION);
                // Verificar si la extensión del archivo está en la lista de extensiones a eliminar
                if (in_array($extension, $extensions_to_delete)) {
                    // Eliminar el archivo
                    File::delete($file);
                }
            }
            ...
            // compilar el archivo bib
            $process2 = new Process(['/usr/bin/biber', "-f", public_path('articles_public/' . $article->id . '/' . $article->id)]);
            $process2->run();
            ...
            if (file_exists(public_path("articles_public/{$article->id}/{$article->id}.pdf"))) {
                ...
                add_packages2($new_file_name);

                // sustituir el FUNDATION por el contenido de my_tex_content
                replace_tex_content2($new_file_name, $my_tex_content);
                ...
                // compilar el archivo bib
                $process2 = new Process(['/usr/bin/bibtex', public_path('articles_public/' . $article->id . '/' . $article->id . '.aux')]);
                $process2->run();
                ...
                if (file_exists(public_path("articles_public/{$article->id}/{$article->id}.pdf"))) {
                    ...
                    // File::deleteDirectory(public_path('articles_public/' . $article->id));

                    // redirigir a la vista de edición del artículo con un mensaje de error
