           ...
           {{-- form para cambiar el titulo, descripcion, autor, fecha de publicacion, orcid, doi, keywords, abstract, y contenido del articulo --}}
            <form action="{{ route('articles.update', $article->id)}}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <div class="mt-1 w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        
                        @foreach ($coauthors as $coauthor)
                            <div class="block w-full px-4 py-2 @if ($loop->first) rounded-t-lg border-b @elseif ($loop->last) rounded-b-lg @else border-b @endif border-gray-200 cursor-pointer hover:bg-gray-100 hover:text-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-700 focus:text-primary-700 dark:border-gray-600 dark:hover:bg-gray-600 dark:hover:text-white dark:focus:ring-gray-500 dark:focus:text-white">
                                <div class="flex items-center justify-between">
                                    <span>{{ $coauthor->name }}</span>
                                    <button class="px-4 py-2 bg-green-500 text-white rounded-lg ml-auto hover:bg-green-600">Agregar</button>
                                </div>
                            </div>
                        @endforeach
